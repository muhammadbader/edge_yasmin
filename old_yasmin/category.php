<?php
/**
 * Category Archive (updated to match "All Kents" styling exactly)
 * Shows only "product" posts that belong to the current WP category.
 */
get_header();

// Add body class for proper styling
add_filter('body_class', function($classes) {
    $classes[] = 'kent-archive';
    return $classes;
});
?>
...
<div class="producer-layout">
  ...
  <!-- === Sidebar (exactly the same as All Kents) === -->
  ...
  <div class="producer-content">
    <section class="products-section">
      ...
      <!-- Page Title with View Controls (matching kent-archive CSS) -->
      <div class="section-header" dir="rtl">
        <h2 class="section-title" style="text-align: right">
          <?php single_cat_title(); ?>
        </h2>
        
        <!-- Add view controls to match kent-archive styling -->
      ...
      // Fetch ONLY "product" posts in this category (including child categories)
      $products_in_cat = new WP_Query( [
        'post_type'      => 'product',
        'posts_per_page' => -1,
        'orderby'        => 'rand',
        'tax_query'      => [
      ...
      <?php if ( $products_in_cat->have_posts() ) : ?>
        <div class="producer-grid" id="category-products-container">
          <?php while ( $products_in_cat->have_posts() ) : $products_in_cat->the_post(); ?>
            <article <?php post_class( 'producer-card' ); ?>>
              <a href="<?php the_permalink(); ?>" class="card-link">
                ...
                <div class="card-image">
                  ...
                  <!-- Add card overlay for hover effects -->
                  <div class="card-overlay">
                    <span class="overlay-text">צפייה בפרטים</span>
                  </div>
                </div>
                ...
                <div class="card-body">
                  ...
                  <!-- Producers with enhanced styling -->
      ...
      <?php else : ?>
        <section class="empty-state">
          ...
          <p class="empty-desc">כרגע אין מוצרים זמינים בקטגוריה זו.</p>
        </section>
      <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (input) {
    input.addEventListener('input', function() {
      ...
      // Show/hide every <li class="producer-item">
      document.querySelectorAll('.producer-sidebar .producer-item').forEach(item => {
        const name = (item.dataset.name || '').toLowerCase();
        item.style.display = name.includes(term) ? '' : 'none';
      });

      // For each parent with children, check if any child remains visible
  ...
  const producerGrid = document.querySelector('.producer-grid');
  
  if (viewButtons.length > 0 && producerGrid) {
    viewButtons.forEach(button => {
      button.addEventListener('click', function() {
        ...
        // Update grid class
        producerGrid.classList.toggle('view-list', view === 'list');
      });
    });
  }
});
